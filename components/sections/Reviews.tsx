...
/* one quote card on the voltage spine */
function ReviewCard({ r, i, light }: { r: Review; i: number; light: boolean }) {
  return (
    <article className="relative sm:pl-12">
      ...
      >
        ...
        <div className="flex items-center justify-between pt-8">
          ...
          {"when" in r && r.when && <span className="text-xs font-medium text-g600">{r.when}</span>}
        </div>

        <blockquote className={`mt-4 text-[1.02rem] leading-relaxed ${light ? "text-g700" : "text-g200"}`}>
          {r.text}
        </blockquote>

        <figcaption
          className={`mt-6 flex items-center gap-3.5 border-t pt-5 ${light ? "border-black/10" : "border-white/10"}`}
        >
          <span
          ...
          <span>
            <span className={`block text-sm font-semibold ${light ? "text-ink" : "text-paper"}`}>{r.name}</span>
...
/* Real Google reviews (5,0 · 8). Never fabricated, verbatim. Design: editorial
   spread - sticky rating monument left, quotes on a voltage spine right. Shows
   3 by default; the rest reveal on demand. Reused on commercial pages. */
    ...
    <section className={`relative overflow-hidden py-20 md:py-28 ${light ? "on-light" : "bg-s0"}`}>
      ...
      <Container className="relative grid gap-14 lg:grid-cols-[0.85fr_1.15fr] lg:gap-16">
        ...
        <div className="lg:sticky lg:top-28 lg:self-start">
          ...
          />
          <Reveal delay={0.12}>
            <p className={`mt-6 max-w-md text-lg leading-relaxed ${light ? "text-g600" : "text-g300"}`}>{intro}</p>
          </Reveal>
          ...
          <Reveal delay={0.18}>
            <div className="relative mt-10 max-w-sm overflow-hidden chamf chamf-lg bg-blue p-8 shadow-[0_36px_80px_-32px_rgba(1,48,253,0.7)]">
              ...
              <div className="relative mt-6 flex items-center gap-2.5 border-t border-white/20 pt-4">
                ...
                <p className="text-[0.82rem] font-medium text-white/80">Alle reviews: vijf sterren</p>
              </div>
            </div>
          </Reveal>
        </div>
        ...
        <div className="relative">
          <span
            aria-hidden
            className={`absolute bottom-6 left-[7px] top-6 hidden w-px sm:block ${light ? "bg-ink/10" : "bg-white/10"}`}
          />
          <div className="space-y-7">
            {/* always-visible first three */}
            {first.map((r, i) => (
              <Reveal key={r.name} delay={i * 0.08}>
                <ReviewCard r={r} i={i} light={light} />
              </Reveal>
            ))}

            {/* the rest — revealed on demand, staggered */}
            <AnimatePresence initial={false}>
              {expanded &&
                rest.map((r, i) => (
                  ...
                  >
                    <ReviewCard r={r} i={i + INITIAL} light={light} />
                  </motion.div>
                ))}
            </AnimatePresence>
          </div>
          ...
          {hasMore && (
            <Reveal delay={0.1}>
              <div className="mt-10 flex items-center gap-5 sm:pl-12">
                <span className={`h-px flex-1 ${light ? "bg-ink/10" : "bg-white/12"}`} />
                <button
                  type="button"
                  onClick={() => setExpanded((v) => !v)}
                  aria-expanded={expanded}
                  className={`group inline-flex shrink-0 items-center gap-2.5 chamf-sm border px-5 py-3 text-sm font-semibold transition-colors duration-150 ${
                    light
                      ? "border-ink/15 bg-white text-ink hover:border-blue hover:bg-blue hover:text-white"
                      : "border-white/15 bg-s1/60 text-paper hover:border-blue hover:bg-blue hover:text-white"
                  }`}
                >
                  {expanded ? "Toon minder" : `Toon alle ${reviews.count} reviews`}
                  <svg
                    width="15"
                    height="15"
                    viewBox="0 0 24 24"
